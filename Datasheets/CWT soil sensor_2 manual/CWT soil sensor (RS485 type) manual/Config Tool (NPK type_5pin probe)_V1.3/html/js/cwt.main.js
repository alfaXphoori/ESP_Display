...
window.onload = function () {
    ...
    InitSensors();
    if (sensor && sensor.funBtns) {
        for (let i = 0; i < sensor.funBtns.length; i++) {
            let btnIndex = i + 1;
            Id("regType").onchange();
            Id("funBtn" + btnIndex).innerHTML = sensor.funBtns[i].name;
            DisplayById("funBtn" + btnIndex, sensor.funBtns[i].name.length != 0);
            let params = sensor.funBtns[i].params;
            Id("funBtn" + btnIndex).onclick = function () {
                sensor.ReceivedData = DataRecvHandler.ReceivedDataNoDeal;
                for (let j = 0; j < params.length; j++) {
                    //多个参数要等待前面的执行
                    (function (j, oneParam) {
                        setTimeout(function () {
                            if (oneParam) {
                                let data = Modbus.GetWriteRegData(sensor.slaveAddr, oneParam.regAddr, oneParam.regType, oneParam.value, oneParam.valueType);
                                data = BytesToHexStr(data);
                                SendHex(data);
                                AppendLog("TX:" + data);
                            }
